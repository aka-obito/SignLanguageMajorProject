...
MODEL_PATH = "D:\\MAJOR\\4thYProjectCode\\accuratemodel.keras"
...
def create_db():
    ...
    # Step 1: Create 'tokens' table if it does not exist
    c.execute('''
    ...
    # Step 2: Check if 'labels' column exists, if not, add it
    c.execute('''PRAGMA table_info(tokens)''')
    columns = [column[1] for column in c.fetchall()]
    if 'labels' not in columns:
        c.execute('''ALTER TABLE tokens ADD COLUMN labels TEXT''')

    # Step 3: Create 'training_data' table if it does not exist
    c.execute('''
...
if menu == "Real-Time Detection":
    ...
    if st.session_state.admin_logged_in:
        ...
        users = get_all_users()
        if users:
            user_data = [{"Name": user[1], "Email": user[2], "Labels": user[3]} for user in users]
            st.table(user_data)
        else:
        ...
        # Debugging: Show the fetched labels
        st.write("### DEBUG: Fetched Labels")
        st.write(labels)
